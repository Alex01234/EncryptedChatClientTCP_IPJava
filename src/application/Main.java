...
public class Main extends Application {
	...
	//Northern HBox
	private Label hostLabel; private TextField hostField; private Label portLabel; private TextField portField; private Button connectButton;
	//Center HBox
	private TextArea serverArea;
	//Right VBox
	private Label usernameLabel; private TextField usernameField; 
	private Label firstPasswordLabel; private TextField firstPasswordField;
	private Label secondPasswordLabel; private TextField secondPasswordField;
	//Bottom Hbox
	private TextArea messageArea; private Button messageButton;
	...
	*/
	...
	public void start(Stage stage) {
		try {
			...
			northBox.getChildren().addAll(hostLabel, hostField, portLabel, portField, connectButton);
			...
			rightBox.getChildren().addAll(usernameLabel, usernameField, firstPasswordLabel, firstPasswordField, secondPasswordLabel, secondPasswordField);
			...
			the int port is set to the text that has been submitted to the TextFiedld portField,
			the Socket socket is set to an instance of the class Socket with the parameters host and port,
			the ObjectOutputStream is set to an instance of the class ObjectOutputStream with the parameter socket.getOutputStream(),
			the method startReceiver is called and
			a text is appended to the TextArea serverArea that indicates that a connection has been made to the server with the host and port submitted by the user.
			
			If the method checkConncetionDetails returns false, the method displayDialog is called with the parameter of the static String connectionDialogString.
			*/
			...
			the TextArea messageArea is then cleared of text.
			
			An instance of the class Serializable is created using the String message,
			an instance of the class IvParameterSpec is created using the text submitted to the TextField firstPasswordField,
			an instance of the class SecretKeySpec is created using the text submitted to the TextField secondPasswordField,
			an instance of the class Cipher is created with the transformation "AES/CBC/PKCS5PADDING",
			the Cipher is initialized in encryption mode using the IvParameterSpec and SecretKeySpec.
			
			A SealedObject is created using the instance of the class Serializable (the message) and the Cipher
			and the SealedObject is then written to the ObjectOutputStream outStream. 
			*/
	...
	*/
	...
	Using the Cipher, the Object (which is a String) is retrieved and decrypted from the SealedObject.
	The decrypted String is then appended to the TextArea serverArea.	
	*/
	private void receivingTask() {
        while(true){
            try{
            	...
            	System.out.println("\n" + "An Exception occured receiving message: " + ex);
            	ex.printStackTrace();
            }
        }
	}//receivingTask
	...
	*/
	private boolean checkConnectionDetails() {
		if(hostField.getText() == null || hostField.getText().trim().isEmpty() || portField.getText() == null || portField.getText().trim().isEmpty()) {
	...
	*/
	...
	If one of the methods above returns false, the method displayDialog is called with the parameter of the String dialog and the method checkAllDetails returns false.
	Otherwise, the method checkAllDetails returns true.
	*/
	private boolean checkAllDetails() throws UnsupportedEncodingException {
		...
		String dialog = "";
		if(!connectionCheck) {dialog = dialog + connectionDialogString;}
		if(!usernameCheck) {dialog = dialog + usernameDialogString;}
		if(!passwordCheck) {dialog = dialog + passwordDialogString;}
		if(!passwordBitCheck) {dialog = dialog + passwordBitString;}
		if(!messageCheck) {dialog = dialog + messageDialogString;}
		if(!connectionCheck || !usernameCheck || !passwordCheck || !passwordBitCheck || !messageCheck) {
